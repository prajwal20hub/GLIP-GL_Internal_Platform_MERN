...
import {
  ...
  TableContainer2,
} from "./task-management-admin.styles";
...
const  TaskManagementAdmin = () => {
  ...
  const viewDetails = async (id) => {
    await axios(`${Base_URL}/api/users/${id}`, {
      headers: {
        'Authorization': `Bearer ${localStorage.token}`          //for verification (IMP)
      }
    })
  ...
  const ChangeDesignation = async (id) => {
    ...
    if (selectOption !== currUser.designation) {
      await axios.put(`${Base_URL}/api/users/designation/${id}`, {
